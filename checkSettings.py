...
def checkChannelSettings(simSettings: simulationSettings):
    ...
    for iteration, filename in enumerate(simSettings.channel.fileNamesNEXT):
        if (filename[-4:] != '.s4p') and (filename[-4:] != '.mat'):
            print('Invalid file extension for NEXT file {0:d}, with filename of "{1}"'.format(iteration, filename))
            error('unknown channel file type detected, please ensure using .s4p or .mat files!')
...
def checkSimTime(simSettings: simulationSettings):
    ...
    if signalingMode == 'QAM' or signalingMode == 'clock':
    ...
    if totalTime > 5*60 :
        ...
        answer = input('The simulation is expected to take {:d} minutes. Are you sure you would like to continue? (y/n)'.format(totalTime/60))
